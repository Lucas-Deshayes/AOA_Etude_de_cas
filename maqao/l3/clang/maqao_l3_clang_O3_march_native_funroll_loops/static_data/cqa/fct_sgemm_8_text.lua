  ...
  paths = {
    {
      ...
      expert = {
        ...
        },
        {
          ...
          txt = "Assuming all data fit into the L1 cache, each call to the function takes 17.75 cycles. At this rate:\n - 3% of peak load performance is reached (2.25 out of 64.00 bytes loaded per cycle (GB/s @ 1GHz))\n - 8% of peak store performance is reached (2.70 out of 32.00 bytes stored per cycle (GB/s @ 1GHz))\n",
        },
        {
      ...
      gain = {
        {
          ...
          txt = "Detected a slowdown caused by scalar integer instructions (typically used for address computation).\nBy removing them, you can lower the cost of an iteration from 17.75 to 2.75 cycles (6.45x speedup).",
        },
        {
          workaround = " - Try another compiler or update/tune your current one\n - Make array accesses unit-stride:\n  * If your function streams arrays of structures (AoS), try to use structures of arrays instead (SoA):\nfor(i) a[i].x = b[i].x; (slow, non stride 1) => for(i) a.x[i] = b.x[i]; (fast, stride 1)\n",
          details = "All SSE/AVX instructions are used in scalar version (process only one data element in vector registers).\nSince your execution units are vector units, only a vectorized function can use their full power.\n",
          title = "Vectorization",
          txt = "Your function is not vectorized.\nOnly 19% of vector register length is used (average across all SSE/AVX instructions).\nBy vectorizing your function, you can lower the cost of an iteration from 17.75 to 2.87 cycles (6.17x speedup).",
        },
        {
